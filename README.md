...
This is actually supervised learning. From the past data(the transformed state) and labels(the action taken by A* player ), we can use MLP model to learn the strategy for our agents.
...
- Each turn the agent get -1 point
- The agent eat enemy sheep with get 100 points
![](https://github.com/lineojcd/Multi-Agent-RL-for-the-Wolf-Sheep-Game/blob/main/src/ddpg.png)
The process of RL is shown in the picture. Still, we use the feature transformation Φ(St) instead of the real St to avoid the heavy computational cost.

#### Proximal Policy Optimization (PPO) player
[Proximal Policy Optimization (PPO)](https://openai.com/research/openai-baselines-ppo): the advantages can be enumerated as follows:
...
#### Please check the detailed usage of PPO and Teacher-Student framework in our paper [here](https://github.com/lineojcd/Multi-Agent-RL-for-the-Wolf-Sheep-Game/blob/main/report/EE260_RL_final_paper.pdf)
